		...
		if (isset($_POST['username']) && isset($_POST['password_'])) {
			...
			if (empty($username)) {
				...
				if (mysqli_num_rows($result) > 0) {
					...
					if ($row['username'] === $username && $row['password_'] === $pass && $row['positionname'] === 'Faculty') {
						$_SESSION['username'] = $row['username'];
						$_SESSION['positionname'] = "Faculty";
						$_SESSION['id'] = $row['adminID'];
						$_SESSION['lastname_'] = $row['lastname_'];
						$_SESSION['firstname_'] = $row['firstname_'];
						$_SESSION['mi_'] = $row['mi_'];
						if ($row['positionname'] === 'Faculty') {
							header("location: announcement_teacher.php");
						}
						elseif ($row['positionname'] === 'Student') {
							header("location: announcement_stud.php");
						}
